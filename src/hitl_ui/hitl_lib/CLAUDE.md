...
| 파일 | 책임 |
|------|------|
| `auth.py` | ALB `x-amzn-oidc-data` (ES256 JWT) **서명 검증** 후 `current_user()`/`current_groups()`/`require_group()` 제공. JWT 헤더의 `kid` 로 ALB region 공개키(`public-keys.auth.elb.{region}.amazonaws.com/{kid}`)를 `lru_cache` fetch 해 검증. **signer 게이트(ADR-011)**: `ALB_ARN` env 설정 시 JWT 헤더 `signer != ALB_ARN` 이면 키 fetch 전에 거부 — region 내 다른 ALB 가 민팅한 토큰 차단. PyJWT/cryptography 없고 `LOCAL_DEV≠1` 이면 **fail-closed**(빈 claims). |
...
- **auth 는 fail-closed**: 서명 검증 실패·kid 누락·공개키 fetch 실패·signer 불일치 → 빈 dict → `current_groups()=[]` → `require_group()` 차단. fail-open(미검증 통과) 금지 (CWE-754). `LOCAL_DEV=1` 만 미검증 디코드 허용.
