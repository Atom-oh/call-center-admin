...
### Git
- **main 브랜치 직접 작업** (현재 단일 워크플로). PR 단위는 commit 단위로 추적.
- Conventional commit 메시지: `feat(scope): ...`, `fix(scope): ...`, `docs(scope): ...`, `test(scope): ...`, `refactor(scope): ...`.
- 메시지 본문에 Co-Authored-By 라인 자동 제거(`scripts/install-hooks.sh`로 설치되는 commit-msg hook이 처리).
