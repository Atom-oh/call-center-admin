...
조직에 이미 `AWS-Demo-Platform` repo 의 EKS hub cluster 에 **Atlantis** 가 설치되어 다른 프로젝트들의 Terraform 을 처리. 본 repo 도 합류 시 운영 통일.
...
**Atlantis 구성**:
- IRSA: `AtlantisIRSARole` (EKS service account → IAM role)
- AssumeRole 체인: AtlantisIRSARole → `DemoPlatformTerraformer` (atomoh-main 계정 180294183052) with `ExternalId`
...
```mermaid
flowchart TD
    PR[GitHub PR open / push] --> WH[GitHub webhook<br/>→ atlantis.atomai.click/events]
    WH --> AT[Atlantis<br/>EKS hub cluster]
    AT --> ART[atlantis.yaml<br/>project: dev<br/>dir: infra/envs/dev]
    AT --> IR[IRSA<br/>AtlantisIRSARole]
    IR --> AR[AssumeRole<br/>DemoPlatformTerraformer<br/>with ExternalId]
...
### Negative
- Atlantis cluster 자체가 SPOF — `AWS-Demo-Platform` 운영팀 의존성. cluster 장애 시 모든 repo terraform 정지.
...
- IRSA role: `AtlantisIRSARole` in EKS hub cluster, trust policy `pod-identity-association`
- AssumeRole target: `arn:aws:iam::180294183052:role/DemoPlatformTerraformer` (`infra/envs/dev/variables.tf:terraformer_role_arn` default)
- GitHub App: `atomoh-atlantis` (installation_id 135720704), webhook `https://atlantis.atomai.click/events`
