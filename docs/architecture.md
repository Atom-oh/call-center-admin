...
#### Query / Presentation (Phase 1 partial, full in PR7-8)
- **Athena workgroup `callcenter-<env>`** + Glue table `consult_results` (PR7)
- **QuickSight Enterprise** — 5 sheets (overview, drill-down, agent heatmap, quality, alerts) (PR7)
- **Streamlit on Fargate behind internal ALB + Cognito** — HITL review queue, search, compliance pages (PR8)
...
- **Cognito** — User Pool with 3 groups (`ops`, `analyst`, `compliance`); ALB authenticate-cognito (PR8).
...
| `classify-pipeline` | per-Lambda staging-dir packaging, IAM, log groups, Lambda ×4, SFN Express, EventBridge | `infra/modules/classify-pipeline/` |
| `analytics` (PR7) | Glue, Firehose Parquet, Athena workgroup, QuickSight datasets | _pending_ |
| `hitl-ui` (PR8) | Fargate, internal ALB, Cognito | _pending_ |
| `observability` (PR9) | CW dashboards, alarms, SNS, Slack relay Lambda | _pending_ |
...
- (Pending) `docs/runbooks/bedrock-throttling.md`, `hitl-backlog.md`, `prompt-rollback.md`, `pii-mask-failure.md` — to be authored in PR10
...
#### 조회·표시 (Phase 1 일부, PR7-8에서 완성)
- **Athena workgroup `callcenter-<env>`** + Glue 테이블 `consult_results` (PR7)
- **QuickSight Enterprise** — 5개 시트 (개요, 드릴다운, 상담원 heatmap, 품질, 알람) (PR7)
- **Streamlit on Fargate behind internal ALB + Cognito** — HITL 검수 큐, 검색, 컴플라이언스 페이지 (PR8)
...
- **Cognito** — User Pool + 3 그룹 (`ops`, `analyst`, `compliance`); ALB authenticate-cognito (PR8).
...
| `classify-pipeline` | per-Lambda staging-dir 패키징, IAM, log group, Lambda ×4, SFN Express, EventBridge | `infra/modules/classify-pipeline/` |
| `analytics` (PR7) | Glue, Firehose Parquet, Athena workgroup, QuickSight 데이터셋 | _보류_ |
| `hitl-ui` (PR8) | Fargate, internal ALB, Cognito | _보류_ |
| `observability` (PR9) | CW dashboard, 알람, SNS, Slack relay Lambda | _보류_ |
...
- (보류) `docs/runbooks/bedrock-throttling.md`, `hitl-backlog.md`, `prompt-rollback.md`, `pii-mask-failure.md` — PR10에서 작성
